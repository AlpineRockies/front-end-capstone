import React, { useState, useEffect } from 'react';
import axios from 'axios';
import './style.css';
import Related from './Related.jsx';
import Comparison from './Comparison.jsx';

export const RelatedItemsImg = React.createContext();
export const RelatedItemsDetails = React.createContext();

function RelatedItemsHooks() {
  const [productId, setProductId] = useState(38013);
  const [relatedItems, setRelatedItems] = useState([]);
  const [relatedItemsImg, setRelatedItemsImg] = useState([]);
  const [relatedItemsDetails, setRelatedItemsDetails] = useState([]);
  const [outfit, setOutfit] = useState([]);

  useEffect(() => {
    axios
      .get(`/products/${productId}/related`)
      .then((res) => setRelatedItems(res.data))
      .catch((err) => console.error(err));
  }, [productId]);

  useEffect(() => {
    Promise.all(
      relatedItems.map((product) =>
        axios
          .get(`/products/${product}/styles`)
          .then((response) => response.data)
          .catch((err) => <div>There was an API error: {err}</div>)
      )
    )
      .then((response) => setRelatedItemsImg(response))
      .catch((err) => console.error(err));
  }, [relatedItems]);

  useEffect(() => {
    Promise.all(
      relatedItems.map((product) =>
        axios
          .get(`/products/${product}`)
          .then((response) => response.data)
          .catch((err) => console.error(err))
      )
    ).then((res) => setRelatedItemsDetails(res));
  }, [relatedItems]);

  // useEffect(() => {
  //   axios
  //     .get(`/products/${productId}/related`)
  //     .then((res) => {
  //       console.log('Related Products array: ', res.data);
  //       return res.data;
  //     })
  //     .then((relatedProductArray) => {
  //       () => setRelatedItems(relatedProductArray);
  //       return relatedProductArray;
  //     })
  //     .then((relatedProductArray) =>
  //       Promise.all(
  //         relatedProductArray.map((product_id) =>
  //           axios
  //             .get(`/products/${product_id}/styles`)
  //             .then((response) => {
  //               console.log('Promise All  -  response.data: ', response.data);
  //               return response.data;
  //             })
  //             .catch((err) => {
  //               console.log(err);
  //             })
  //         )
  //       )
  //     )
  //     .then((promiseArray) => {
  //       console.log('Related Styles Array: ', relatedItemsImg, promiseArray);
  //       setRelatedItemsImg(promiseArray);
  //       return promiseArray;
  //     })
  //     .then((promiseArray) =>
  //       Promise.all(
  //         promiseArray.map((product) =>
  //           axios
  //             .get(`/products/${product.product_id}`)
  //             .then((response) => response.data)
  //             .catch((err) => {
  //               console.log(err);
  //             })
  //         )
  //       )
  //     )
  //     .then((response) => {
  //       setRelatedItemsDetails(response);
  //     })
  //     .catch((err) => {
  //       console.log('err', err);
  //     });
  // }, []);

  const fetchData = () => {};

  const handleClick = (event) => {
    console.log('[RelatedItems] handleClick :', event);
    // feed this item back to app for reload of Overview
    getYourOutfit(event);
  };

  const getYourOutfit = (event) => {
    console.log('[getYourOutfit] ReleatedItems.jsx', event);
    // axios
    //   .get(`/products/${product_id}/styles`)
    //   .then((response) => response.data)
    //   .then((prod) => {
    //     console.log('YOUR OUTFIT: ', prod);
    //   })
    //   .catch((err) => console.log(err));
  };

  return (
    <div className="ri-parent">
      <div className="ri-relateditems">
        <h3>Related Items</h3>
        <RelatedItemsImg.Provider value={relatedItemsImg}>
          <RelatedItemsDetails.Provider value={relatedItemsDetails}>
            <Related setProductId={setProductId} />
          </RelatedItemsDetails.Provider>
        </RelatedItemsImg.Provider>
      </div>
      <div className="ri-comparison">
        <h3>Comparison</h3>
        <Comparison />
      </div>
    </div>
  );
}

export default RelatedItemsHooks;

//<Related relatedItems={relatedItemsImg} handleClick={handleClick} />
